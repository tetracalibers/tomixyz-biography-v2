...
import img_WebGPUTriangleVertexBuffer from "../../../assets/tech/webgpu-concept/webgpu-triangle.png"
...
頂点シェーダの入力となる`VertexInput{:wgsl}`構造体を、次のように変更します。

```wgsl title="頂点シェーダの入力として頂点座標を受け取る" {2}
struct VertexInput {
  @location(0) position: vec2f
};
```

`@location(0){:wgsl}`がどんな意味を持つかは、この後すぐに解説します。

`vec2f{:wgsl}`は、WGSLにおける2次元ベクトルの型であり、`f32{:wgsl}`型の数値を2つ並べたものを1つの頂点座標として扱うことを意味しています。

これで、`VertexInput{:wgsl}`構造体の中に`position{:wgsl}`という変数を定義できました。
この`position{:wgsl}`変数を頂点座標として使うように、頂点シェーダの本体である`vs_main{:wgsl}`関数も変更しておきましょう。

```wgsl title="頂点シェーダ関数内で頂点座標を使う" {4}
@vertex
fn vs_main(in: VertexInput) -> VertexOutput {
  var out: VertexOutput;
...
まずは、1つの頂点を表すデータの並びの説明である==属性デスクリプタ==を定義します。

```js title="頂点属性のデスクリプタを定義する"
...
`shaderLocation{:js}`は、このデータをシェーダ側の「どの変数に入れるか」を指定するものです。
`shaderLocation: 0{:js}`と指定した場合は、WGSLシェーダ側の`VertexInput{:wgsl}`構造体のうち、`@location(0){:wgsl}`という印をつけた変数にデータが格納されます。

`offset{:js}`は、1つのバッファ内に複数の属性（頂点座標、色など）を含めた場合に、各属性がバッファ内のどの位置から始まるかを指定するものです。
今回は頂点座標だけを格納しているため、あまり気にする必要はありません。`0{:js}`と指定しておけば、バッファの先頭から読み出しを始めることになります。

`format{:js}`は、データの型を指定するものです。`float32x2{:js}`という指定は、32ビット浮動小数点数（`f32{:wgsl}`）を2つ並べたものを1つの頂点として扱うことを意味しています。
これは、WGSLシェーダ側で`position{:wgsl}`変数の型を`vec2f{:wgsl}`と指定したことと対応しています。

このように、1つの頂点を表すデータの構成を決めたら、今度は頂点バッファ全体のデータの並びの説明である==レイアウトデスクリプタ==を定義します。

```js title="頂点バッファのレイアウトデスクリプタを定義する"
...
`arrayStride{:js}`は、1つの頂点を表すデータがバッファ内でどれくらいのバイト数を占めるかを指定します。
この指定により、たとえば2番目の頂点を取り出すには、バッファの先頭から`arrayStride{:js}`バイトだけ進んだ位置から読み出せばよいことになります。

`vertices{:js}`は、頂点座標データを格納したJavaScriptの`Float32Array{:js}`型の配列でした。
この`BYTES_PER_ELEMENT{:js}`プロパティは、配列内の1つの数値が何バイトであるかを示すプロパティです。
ここでは、1つの頂点を2つの数値で表現しているため、`2{:js}`をかけることで1つの頂点のデータが占めるバイト数を計算しています。

`attributes{:js}`には、1つの頂点に含まれる属性のデスクリプタを配列としてまとめて指定します。

これで、頂点バッファ全体のデータの並びを定義することができました。
このレイアウトデスクリプタを、描画設定を一元管理するオブジェクトである==レンダーパイプライン==に組み込むことで、シェーダは頂点バッファのデータを正しく読み取ることができるようになります。

```js title="レンダーパイプラインにレイアウトデスクリプタを組み込む" {6-8}
...
ここまでの説明では、役割を明確にするために`positionAttribute{:js}`や`vertexBufferLayout{:js}`を変数として定義してきましたが、わざわざ変数にせず、次のようにレンダーパイプラインに直接書き込んでしまってもよいでしょう。

```js title="レンダーパイプラインにレイアウトデスクリプタを組み込む" {6-17}
const renderPipeline = device.createRenderPipeline({
  layout: "auto",
  vertex: {
    module: shaderModule,
...
```js title="レンダーパスで頂点バッファを使って描画する" {2}
...
`setVertexBuffer(){:js}`メソッドの第一引数として渡している`0{:js}`は、`renderPipeline.vertex.buffers{:js}`配列のうち、`0{:js}`番目のバッファを使うことを意味しています。
