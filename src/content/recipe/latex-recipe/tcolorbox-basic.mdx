...
import img_EmptyVsMinimal from "../../../assets/recipes/tcolorbox-basic/empty-vs-minimal.png"

- TODO: LaTeXで数学ノートを作るにあたって、独自の囲み枠〜
...
tcolorboxパッケージは、囲み枠付きでコンテンツを描画する`tcolorbox`環境を提供します。
書式は次の通りです。
...
<FigureBox src={img_TcbDefault} alt="" />

ここからさらにオプションを指定することで、タイトルをつけたり、スタイルをカスタマイズしたりすることができます。
...
デフォルトの余白は`size=minimal`オプションで、デフォルトの装飾は`empty`オプションで消すことができます。

<FigureBox src={img_EmptyVsMinimal} alt="" />

```latex showLineNumbers {3-5,9-11,15-17}
\lipsum[1][1-2]
...
先ほどのコード例で使用した`\lipsum`コマンドは、ダミーテキストを出力するコマンドです。

```latex showLineNumbers {1,7,13,19}
\lipsum[1][1-2]
...
`\lipsum`コマンドを使うには、プリアンブルに次の指定を加える必要があります。
...
また、ページをまたぐほど長い文章を入れたときの挙動はどうなるのか調べたい時など、文章の長さを変えながら表示を調整する際に`\lipsum`コマンドを使うと便利です。
