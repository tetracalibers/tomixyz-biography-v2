---
title: tcolorboxのカスタムボックス実装事始め
date: "2025-08-30"
description: skin切り替え・余白指定・デバッグ・リセットなどの基本
tags:
  - latex
  - tcolorbox
series: latex-recipe
...
ボックスの内側の余白と枠線の太さを制御するオプションには、次のようなものがあります。

- 枠線の太さ：`toprule`/`bottomrule`/`leftrule`/`rightrule`
- 内側の余白：`boxsep`/`top`/`bottom`/`left`/`right`

枠線の太さはその名の通り、上下左右に対応する`~rule`オプションでそれぞれ指定します。

`boxsep`は、ボックスの中身と枠の間に、共通のパディング（余白）を設定するオプションです。
`boxsep`でベースとなる余白を指定したら、さらに`left`/`right`/`top`/`bottom`オプションで上下左右の余白を個別に加算する仕組みになっています。
