...
これは、前回実装したシェーダのコードの中で、`@vertex`という印をつけて定義した関数の名前です。
...
このレンダーパイプライン（描画設定）で描画を行うには、`renderPass.draw(){:js}`メソッドを呼び出す前に、同じく`renderPass`が持つ`setPipeline(){:js}`メソッドによって、レンダーパイプラインを使用するように指示します。
