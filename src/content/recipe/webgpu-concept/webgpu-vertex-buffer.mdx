...
> 実際は、GPU側のメモリに直接書き込むことはできません。CPUからアクセスできる中間メモリ（==ステージングバッファ==）に対してマッピング・書き込みを行い、そこからGPU側のメモリにコピーするという仕組みになっています。
...
しかし、GPUはそれを「色」や「座標」として直接理解してくれるわけではありません。「このバッファのこの位置には、何のデータが入っているのか」を一緒に伝える必要があります。

これが「レイアウト（layout）」です。たとえば「1つの頂点には、位置が3つ（X, Y, Z）、色が4つ（R, G, B, A）あります」というような情報を、プログラム側で教えてあげることで、GPUはバッファの中身を正しく解釈できます。

- 属性記述子：ひとつの属性（例：位置や色）の「場所（`@location`）」や「データ型（`vec3<f32>` など）」を記述するものです。
- レイアウト記述子：多くの頂点がまとめて格納されたバッファから、「各頂点のデータをどう取り出すか」というルールを定義するものです。

GPU に頂点データを送るとき、通常は 複数の頂点をまとめた大きなバッファ を一気に送信します。
第1章で説明したように、少量のデータを小分けで送るのは効率が悪いため、バッチ送信が基本です。

このバッファには、各頂点の 位置、色、テクスチャ座標 などが混在して入っている場合もあります。
また、属性ごとに別々のバッファを用意することも可能です。

一方、頂点シェーダーが実行される時点では「一つの頂点ずつ」しか処理されません。
つまり、バッファ全体の情報を一度に見ることはできません。
このような並列処理に対応するために、GPU は「各頂点に必要なデータ」をバッファから 自動的に切り出す 仕組みを持っており、そのために レイアウト記述子の情報が必要になります。
