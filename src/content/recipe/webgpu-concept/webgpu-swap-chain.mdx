...
今回は、WebGPUによるキャンバス描画とは、その裏側でどのような処理が行われているのか、もう少し深掘りしていきます。
具体的に注目するのは、`canvas{:html}`要素から取得したコンテキストを中心とする、次のメソッドです。

- `gpu.getPreferredCanvasFormat(){:js}`は何を取得しているのか？
